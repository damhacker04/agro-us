...
AgroUs punya fondasi antarmuka yang konsisten: bahasa Indonesia, navigasi ponsel, komponen formulir berlabel, status yang dijelaskan lewat teks, pratinjau konsekuensi panen, serta data biaya dan status yang sebagian memakai kontrak bersama. Produk cocok sebagai **prototipe lomba yang punya pembeda kuat**, tetapi **belum memenuhi klaim seluruh flow berfungsi 100% atau siap melayani pengguna baru secara mandiri**. Hambatan paling nyata justru di jalur paling awal: profil pengguna baru tidak dihubungkan setelah OTP dan pilihan zona pembeli tidak tersimpan ke sumber data yang dipakai checkout.
...
Prioritas P1 = jalur inti terputus/risiko sesi atau transaksi; P2 = fungsi wajib parsial, data menyesatkan atau friksi besar; P3 = pemeliharaan/polish. Semua temuan masih terbuka kecuali laporan utama menyatakan perbaikan berikutnya.

| ID | Prioritas | Temuan dan dampak | Bukti sumber | Saran konkret |
|---|---|---|---|---|
| FE-01 | P1 | Tenant baru selesai OTP menuju `/tenant`, tetapi profil usahanya belum ada. Halaman onboarding tersedia namun tidak menjadi kelanjutan registrasi. Seed demo menyembunyikan masalah ini. | `apps/web/src/app/auth/verify/page.tsx:89`; `apps/web/src/lib/auth.ts:43`; `apps/api/src/modules/auth/auth.service.ts:165` membuat User saja dan mengembalikan `isNewUser`; `apps/web/src/app/tenant/(dashboard)/page.tsx:61` langsung mengambil data tenant. | Routing berdasarkan status profil, bukan hanya role atau `isNewUser`; resume onboarding yang belum selesai saat login ulang. |
| FE-02 | P1 | Pembeli baru tidak punya layar membuat profil usaha. Wrapper `buatProfilPembeli` tidak dipanggil halaman mana pun, sehingga pengguna baru tidak dapat memesan. | `apps/web/src/lib/api.ts:239`; `apps/api/src/modules/buyer/buyer.service.ts:76` menolak jika profil belum ada. Pencarian semua `apps/web/src` hanya menemukan definisi wrapper. | Tambahkan profil usaha dan zona setelah OTP, lalu uji end-to-end menggunakan nomor baru, bukan akun seed. |
| FE-03 | P1 | “Ganti wilayah” hanya mengubah query katalog; checkout memakai `buyer.activeZoneId` dari database. Pengguna dapat melihat zona B sementara ongkir, minimum, dan kecocokan pesanan dievaluasi terhadap zona A. | `apps/web/src/app/buyer/region/page.tsx:85`; `apps/web/src/lib/api.ts:242` tidak dipanggil halaman; `apps/api/src/modules/order/order.service.ts:163` dan `:188`. | Simpan zona melalui PATCH profil sebelum navigasi; tentukan kebijakan keranjang saat pindah zona dan tampilkan zona pengiriman yang benar pada ringkasan. |
| FE-04 | P1 | Keluar pada pembeli dan operator hanya pindah halaman; token tetap ada dan dipakai API. Pengguna berikutnya pada perangkat bersama berisiko memakai sesi lama. | `apps/web/src/app/buyer/(dashboard)/layout.tsx:61`; `apps/web/src/app/operator/(dashboard)/layout.tsx:47`; bandingkan tenant yang memanggil `hapusSesi` pada layout. | Satu fungsi sign-out untuk semua peran, hapus kredensial dan state pribadi, lalu ganti history. Tambahkan uji klik logout. |
| FE-05 | P2 | Keranjang disimpan pada satu kunci global, tidak dipisah pengguna/zona. Bahkan `hapusSesi` tidak menghapusnya. Login pembeli B mewarisi pilihan pembeli A. | `apps/web/src/lib/keranjang.ts:17`; `apps/web/src/lib/auth.ts:37`. Reproduksi FE-REG-05. | Namespace per akun atau reset eksplisit saat pergantian akun; kebijakan zona harus jelas. |
...
Konfigurasi memasukkan seluruh `src/**/*.{ts,tsx}`, termasuk halaman bisnis yang belum diuji; hanya test dan helper test dikecualikan. Coverage **baris/statements17,21% (2.163/12.568)**, **fungsi67,88% (186/274)** dan **cabang91,35% (507/555)**. Halaman checkout/payment/wilayah dan PenilaianPanen mencapai100% dalam pengukuran khusus; PetaLahan memiliki satu guard internal belum terjangkau. Tidak ada penyempitan denominator demi klaim100%; mayoritas halaman tetap memerlukan pengujian. Lihat [hasil lengkap](TEST_RESULTS.md). Coverage penuh tidak membuktikan semua kombinasi input, jaringan, browser, keamanan atau kebutuhan pengguna benar.
