...
/**
 * `shipment.updateMany` meniru UPDATE bersyarat: statusnya disimpan di satu tempat dan
 * hanya berpindah kalau status saat ini masih termasuk yang diizinkan. Tanpa itu, uji
 * balapan di bawah hanya akan membuktikan bahwa mock bisa dipanggil dua kali.
 */
function fixture(initialStatus = "TIBA_DI_LOKASI") {
  const state = { status: initialStatus, receivedMode: null as string | null, claimWindowEndsAt: null as Date | null };
  const tx = {
    shipment: {
      updateMany: vi.fn(async ({ where, data }: { where: { status: { in: string[] } }; data: Record<string, unknown> }) => {
        ...
        state.receivedMode = data.receivedMode as string;
        state.claimWindowEndsAt = data.claimWindowEndsAt as Date;
        return { count: 1 };
      }),
    },
    trackingSession: { updateMany: vi.fn() },
  };
  const prisma = { shipment: { findFirst: vi.fn(async (_args: { where: Record<string, unknown> }) => ({ id: "s1", status: state.status }) as { id: string; status: string } | null), findMany: vi.fn().mockResolvedValue([]), findUnique: vi.fn(async () => ({ id: "s1", arrivedAt: null as Date | null, ...state }) as Record<string, unknown> | null) }, $transaction: vi.fn(async work => work(tx)), $queryRaw: vi.fn().mockResolvedValue([]), trackingSession: { findFirst: vi.fn().mockResolvedValue(null) } };
  ...
  return { prisma, tx, state, gateway, service: new PodService(prisma as never, gateway as never, window as never) };
}
...
describe("PodService dual-signal receipt", () => {
  it.each(["TIBA_DI_LOKASI", "DIKIRIM"])("allows buyer confirmation from %s and starts the standard claim window", async status => {
    const { service, prisma, tx, gateway } = fixture(status);
    ...
    expect(tx.shipment.updateMany.mock.calls[0]![0]).toMatchObject({ where: { id: "s1", status: { in: ["TIBA_DI_LOKASI", "DIKIRIM"] } }, data: { podPhotoUrl: "/proof.jpg", receivedMode: "BUYER_CONFIRM" } });
  ...
  it("auto-accepts only geofence arrivals older than one hour with a 24-hour claim window", async () => {
    ...
    expect(tx.shipment.updateMany.mock.calls[0]![0]).toEqual({ where: { id: "s1", status: { in: ["TIBA_DI_LOKASI"] } }, data: { status: "DITERIMA", receivedMode: "AUTO_60MIN", claimWindowEndsAt: new Date(now.getTime() + CLAIM_WINDOW_FALLBACK_MS) } });
  ...
  it("reports absent location honestly and preserves the no-GPS flag", async () => {
    const { service, prisma } = fixture("DIKIRIM");
  ...
  it("commits and emits only once for simultaneous buyer confirmations (BE-04)", async () => {
    const { service, gateway, tx } = fixture();
    const results = await Promise.all([service.confirmReceipt("u1", "s1", "/proof.jpg"), service.confirmReceipt("u1", "s1", "/proof.jpg")]);
    expect(gateway.emitStatus).toHaveBeenCalledTimes(1);
    expect(tx.trackingSession.updateMany).toHaveBeenCalledTimes(1);
    // Kedua ketukan melihat SATU tenggat klaim yang sama — bukan dua tenggat berbeda
    // yang nanti diperdebatkan saat klaim mutu masuk.
    expect(results[0]).toEqual(results[1]);
  });
  it("returns the stored receipt when auto-accept won the race (BE-04)", async () => {
    const { service, prisma, state, gateway } = fixture();
    // Auto-terima menang lebih dulu; jendelanya 24 jam, bukan 2 jam.
    prisma.shipment.findMany.mockResolvedValue([{ id: "s1" }]);
    expect(await service.autoAcceptStale()).toEqual({ autoAccepted: 1 });
    // Ketukan pembeli sudah terlanjur melewati pemeriksaan awal dengan status lama.
    prisma.shipment.findFirst.mockResolvedValue({ id: "s1", status: "TIBA_DI_LOKASI" });
    const stored = new Date(now.getTime() + CLAIM_WINDOW_FALLBACK_MS);
    expect(await service.confirmReceipt("u1", "s1", "/proof.jpg")).toEqual({ shipmentId: "s1", status: "DITERIMA", receivedMode: "AUTO_60MIN", claimWindowEndsAt: stored.toISOString() });
    expect(state.claimWindowEndsAt).toEqual(stored);
    expect(gateway.emitStatus).toHaveBeenCalledTimes(1);
  });
  it("asks the caller to reload when the status moved elsewhere mid-confirmation", async () => {
    const { service, prisma, state } = fixture();
    // Lolos pemeriksaan awal, lalu status berpindah ke arah lain sebelum UPDATE.
    prisma.shipment.findFirst.mockResolvedValue({ id: "s1", status: "TIBA_DI_LOKASI" });
    state.status = "DIBATALKAN";
    await expect(service.confirmReceipt("u1", "s1", "/proof.jpg")).rejects.toMatchObject({ response: { code: "STATUS_BERUBAH" } });
  });
  it("skips a shipment the buyer already confirmed instead of overwriting its claim window", async () => {
    const { service, prisma, state, gateway } = fixture();
    await service.confirmReceipt("u1", "s1", "/proof.jpg");
    const buyerWindow = state.claimWindowEndsAt;
    prisma.shipment.findMany.mockResolvedValue([{ id: "s1" }]);
    expect(await service.autoAcceptStale()).toEqual({ autoAccepted: 0 });
    expect(state.receivedMode).toBe("BUYER_CONFIRM");
    expect(state.claimWindowEndsAt).toEqual(buyerWindow);
    expect(gateway.emitStatus).toHaveBeenCalledTimes(1);
  });
});
