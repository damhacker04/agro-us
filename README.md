...
| **Penyimpanan foto** | Jalur S3/R2 **sudah siap pakai** dan aktif otomatis begitu `S3_BUCKET`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, dan `S3_PUBLIC_URL` terisi — konfigurasi setengah jalan sengaja menggagalkan boot. Selama keempatnya kosong, penyimpanan mundur ke disk kontainer yang ephemeral: **foto bukti dari demo sebelumnya sudah benar-benar hilang** karena redeploy. Cek yang sedang aktif lewat `GET /operator/penyimpanan` |
